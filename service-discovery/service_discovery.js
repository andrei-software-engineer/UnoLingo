...
app.post('/register', async (req, res) => {
    ...
    try {
        const services = await redisClient.lRange(serviceName, 0, -1); // Use lRange with async/await redis
...
app.get('/services/:name', async (req, res) => {
    ...
    try {
        ...
        if (services.length > 0) {
            ...
            await redisClient.rPopLPush(serviceName, serviceName);  // Use rPopLPush with async/await remove the last element and push it tot front
...
// have pointers and check for first and second list for load balancing

// Start the service discovery server
app.listen(PORT, () => {
    console.log(`Service Discovery running on http://service-discovery:${PORT}`);
});
